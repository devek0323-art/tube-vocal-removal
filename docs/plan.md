...
## v3.01 — 노래방 영상을 접고 가사 조회를 남긴다

P6은 자막 타이밍을 원하는 수준으로 못 맞춰 삭제했다. 정렬 자체는 정확했다(원곡 위에
얹으면 오차 1초 안쪽). 문제는 **정확한 것이 노래방에서는 쓸모없다**는 데 있다. 부르는
순간에 자막이 뜨면 읽을 시간이 0이고, 미리 띄우면 앞 줄이 잘린다. 한 칸에 한 줄만
띄우는 구조로는 둘을 동시에 만족시킬 수 없다.

다시 한다면 글자마다 색이 차오르는 방식이어야 한다. 그러면 "언제 바뀌느냐"가 아니라
"지금 어디를 부르는지"가 보여서 1~2초 오차가 문제가 되지 않는다.

P6을 만들며 얻은 가사 조회 개선(팝업·제목 파싱·동명이곡 방지)은 P1~P5에도 쓰이므로
남겼다. 실측 100곡에서 한글 매칭이 50%에서 95%로 올랐다.
