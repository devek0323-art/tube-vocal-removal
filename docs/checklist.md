...
## v3.01 — P6 결함 수정 + 가사 직접 찾기

### 결함 수정 (코덱스 검토 + 자체 재현)
- [ ] macOS에서 GPU 켜면 nvenc를 골라 영상 실패 → 플랫폼별 인코더 + 실패 시 libx264 폴백
- [ ] 제목에 작은따옴표가 있으면 자막 필터가 깨짐 → 작업 디렉터리 이동 후 고정 파일명만 전달
- [ ] 손상된 위스퍼 모델 파일을 통과시켜 tqdm 크래시 재발 → 기존 파일도 해시 검사
- [ ] whisper_installed가 설정과 다른 모델을 검사 → cfg의 GPU 설정을 따름
- [ ] 가사 조회 예외를 로그 없이 삼킴 → 로그 남김
- [ ] 영상 실패 시 임시 파일과 깨진 MP4가 남음 → finally에서 정리
- [ ] 중단이 ffmpeg를 멈추지 못함 → 프로세스 등록
- [ ] 모델 받는 중에도 분리 시작이 눌림 → 가드 추가
- [ ] 윈도우 예약명(CON·NUL) 제목이면 폴더 생성 실패 → sanitize에서 처리
- [ ] 아주 긴 제목이 배경 카드 밖으로 잘림 → 폭 제한

### 가사 직접 찾기 (A안 — 분리 시작 시 팝업)
- [ ] 분리 시작을 누르면 대기열 전체 가사를 먼저 조회
- [ ] 못 찾은 곡마다 팝업 — 가수·곡명 재검색 / 직접 붙여넣기 / 건너뛰기
- [ ] 고른 가사를 항목에 물고 있다가 변환 때 재사용 (재조회 없음)
- [ ] 길이 대조를 유지하기 위해 제목 확인 시 영상 길이도 저장
