...
## v3.01 — 가사 조회 개선 + P6 삭제

### 가사 직접 찾기 (분리 시작 시 팝업)
- [x] 분리 시작을 누르면 대기열 전체 가사를 먼저 조회
- [x] 못 찾은 곡마다 팝업 — 가수·곡명 재검색 / 직접 붙여넣기 / 건너뛰기
- [x] 고른 가사를 항목에 물고 있다가 변환 때 재사용 (재조회 없음)
- [x] 길이 대조를 위해 제목 확인 시 영상 길이도 저장
- [x] 팝업 검색도 국내 가사 사이트까지 조회

### 가사 매칭 (실측 100곡: 한글 50% → 95%, 영어 92% → 98%)
- [x] 가수·곡 순서가 뒤집힌 제목
- [x] 따옴표 안에만 곡명이 있는 제목
- [x] 괄호에 영문을 병기한 제목 (괄호 안 한글을 곡명으로)
- [x] 한글 자모 `ㅣ`·세로줄 구분자
- [x] 곡명 자리에 `가수 ： 곡`이 통째로 들어간 방송 클립 제목
- [x] 제목 끝의 방송 날짜(20160730)
- [x] 한쪽에만 공백이 붙은 대시 (`조성모 -To Heaven`)
- [x] 같은 제목의 다른 곡 방지 — 모호할 때 가수를 원제목과 대조
- [x] 가사 본문에 섞여 있던 타임코드 제거

### P6(노래방 영상) 삭제
- [x] 자막 타이밍을 쓸 만한 수준으로 못 맞춰 기능을 내림
- [x] 모드·UI·파이프라인·전용 모듈 제거 (P6 자리는 빈 버튼)
- [x] 번들에서 위스퍼·글꼴 제외 (requirements는 그대로 — 런타임 재설치 방지)
- [x] 스모크 검사에서 P6 항목 제외
