...
### 제목 파싱 — 100곡 실측으로 잡은 것

영어 50 / 한글 50의 실제 유튜브 제목으로 가사 매칭률을 쟀다. 정답은 LRCLIB에
가수·곡명으로 직접 조회한 결과와 대조했다.

    영어  92% → 98%      한글  50% → 95.2%

한글이 낮았던 이유는 전부 제목 형태였고, **틀린 가사가 붙은 적은 없었다**(전부 못 찾음).
고친 것은 다섯 가지다.

- 제목 해석을 하나로 고르지 않고 **여러 개 만들어 순서대로 시도**한다. 가수·곡이
  뒤집힌 제목(`우주를 줄게 - 볼빨간사춘기`)이 흔하다.
- 따옴표 안을 곡명 후보로 (`소녀시대 'Gee' MV`). `Girls'`의 아포스트로피를 여는
  따옴표로 오인하지 않도록 앞뒤가 글자가 아닐 때만 본다.
- 괄호 밖이 로마자, 안이 한글이면 안쪽으로 바꾼 변형을 추가한다
  (`Seung-Chul Lee(이승철)` → `이승철`). LRCLIB 국내곡은 한글로 올라와 있다.
- 세로줄을 보조 구분자로 쓴다. `Rainbow | Since You Been Gone`에서 앞부분만 남겨
  **`Rainbow`를 곡명으로** 검색하는 바람에 엉뚱한 한국 곡이 붙고 있었다.
- `ㅣ`(U+3163)는 세로줄이 아니라 한글 자모인데 국내 채널이 구분선으로 쓴다.

측정 도구를 세 번 고쳤다. 싱크 가사의 타임스탬프를 정답으로 쓰고, plainLyrics에
박힌 타임스탬프를 못 걸러내고, 앞 400자만 비교해 정답 머리글에 밀렸다. **결과가
이상하면 대상보다 자를 먼저 의심할 것.** 이 과정에서 plainLyrics의 타임스탬프를
그대로 저장하던 제품 버그도 같이 나왔다.

### 가사 출처는 두 곳 — 둘 다 필요하다

실측(한글 50곡): **LRCLIB 29곡 / 국내 사이트 21곡**. 국내곡의 42%를 폴백이 건진다.

한 곳으로 통일하면 안 된다.

- LRCLIB만 → 국내곡 절반이 날아간다
- 국내 사이트만 → **싱크 가사를 통째로 잃는다.** P6의 빠른 경로가 사라져 모든 곡이
  음성 인식을 타고, 곡당 40~60초가 더 걸린다. HTML을 긁는 방식이라 사이트가 바뀌면
  조용히 죽는 위험도 있다.

새로 만든 팝업 검색이 LRCLIB만 보고 있었다. 자동 조회는 폴백까지 가는데 직접 찾기가
안 가면 앞뒤가 맞지 않는다. 검색 결과에 출처를 함께 표시한다.

### 가사 싱크 — 인식 대신 강제 정렬

받아쓴 결과에 가사를 문자열로 짝짓는 방식은 **잘못 들으면 그 줄이 통째로 끌려간다**.
야생화에서 3번 줄이 −10.3초 튀었다. 위스퍼의 단어 시각을 써도(세그먼트 → 단어)
짧은 우연 일치가 앵커로 잡히면 같은 일이 난다.

방법을 바꿨다. `whisper.timing.find_alignment`은 **임의의 토큰**을 받는다. 받아쓴
결과 대신 **진짜 가사 토큰**을 넣으면 모델은 "이 글자들이 언제 발음됐는지"만
계산한다. 잘못 들을 여지가 없어 그 오류가 구조적으로 발생하지 않는다.

실측(야생화, 9줄): 평균 오차 0.5초, 첫 줄 빼면 0.2초.

주의할 점 두 가지.

- **창의 첫 단어는 창 시작으로 늘어난다.** 앞쪽 오디오를 혼자 떠안기 때문이다.
  바로 앞 줄을 미끼로 함께 넣어 그 몫을 대신 받게 한다. 곡의 맨 첫 줄만은 미끼가
  없어 2~3초 늦는다.
- 위스퍼는 30초 창만 본다. 20초씩 겹쳐 훑고 각 줄은 **신뢰도가 가장 높은 창**의
  결과를 쓴다.

새 의존성도 추가 다운로드도 없다. 이미 싣고 있는 모델의 기능을 제대로 쓰는 것뿐이다.

### 앞부분 가사가 한곳에 몰리던 이유

`빨래`에서 첫 다섯 줄이 20초에 몰려 0.35초 간격으로 쏟아졌다. 사용자가 "앞 가사가
끝나기도 전에 다음이 나온다"고 한 게 이것이다.

원인은 둘 다 앞부분에만 걸리는 조건이었다.

- 강제 정렬의 창 배정이 `창 시작 + 0.5초` 이후만 받았다. 0초에 시작하는 곡의 첫 줄은
  **어느 창에도 못 들어가** 통째로 보간으로 넘어갔다. 첫 창은 앞쪽을 다 받게 했다.
- `vocal_onset`이 이 라이브 녹음에서 20초를 돌려줬고, 그것을 하한선으로 써서 앞 줄들을
  전부 20초 뒤로 밀었다. 확정된 줄보다 뒤인 onset은 믿지 않는다.

두 조건이 겹치면 앞 줄들이 전부 같은 값이 되고, 최소 간격 0.35초가 그것들을 줄줄이
붙여 놓는다. 결과만 보면 "자막이 빠르게 지나간다"로 보이지만 정렬 오차가 아니라
**앞부분이 통째로 버려진** 것이다.

진단은 **원곡(목소리 있는 음원) 위에 자막을 얹어** 확인했다. 반주만 있는 영상으로는
사람이 기억에 의존해 판단하게 되고, 측정 도구도 여러 번 틀렸다. 목소리와 자막을
같이 보여주면 한 번에 판정된다. 다음에도 이 방법을 먼저 쓸 것.

### 노래방 영상(P6)을 뺀 이유

자막 타이밍을 쓸 만한 수준으로 못 맞췄다. 정렬 자체는 정확했다 — 원곡(목소리 있는 음원)
위에 자막을 얹으면 오차 1초 안쪽이고, 사용자도 "엄청 정확하다"고 확인했다.

문제는 **정확한 것이 노래방에서는 쓸모없다**는 데 있었다. 부르기 시작하는 순간에 자막이
뜨면 읽을 시간이 0이다. 미리 띄우면 앞 줄이 잘리고, 안 띄우면 늦는다. **한 칸에 한 줄만
띄우는 구조로는 둘을 동시에 만족시킬 수 없다.**

제대로 하려면 진짜 노래방처럼 **글자마다 색이 차오르는** 방식이어야 한다. 그러면 "언제
바뀌느냐"가 아니라 "지금 어디를 부르는지"가 보여서 1~2초 오차가 문제가 되지 않는다.
강제 정렬이 이미 단어별 시각을 주므로 데이터는 있다. 새로 설계하는 작업이다.

가사 조회 개선(팝업, 제목 파싱, 동명이곡 방지)은 P1~P5에도 쓰이므로 남겼다.

의존성(openai-whisper)은 requirements.txt에 그대로 뒀다. 빼면 RUNTIME_REQUIREMENTS_SHA가
바뀌어 기존 사용자에게 2GB 재설치를 강요하게 된다. 번들에서는 빠지므로(스펙에서 제거)
용량은 줄어든다.
