...
썸네일 주소는 목록에 넣을 때 제목과 함께 받아 두는데(`_resolve_url_titles`), 그
스레드는 `status == "wait"`인 항목만 처리한다. 붙여넣자마자 분리를 시작하면 조건이
깨져 주소가 빈 채로 남는다. 캐시된 파일을 재사용할 때도 마찬가지다.

제목은 다운로드된 파일명에서 따로 채워지므로(`_download` 끝), **제목만 맞고 배경만
빠지는** 상태가 된다. 원인을 좁히는 데 시간이 걸린 이유는 실패를 통째로 삼키고
로그를 남기지 않았기 때문이다. 조용한 실패는 만들지 말 것.

고치는 방향은 "시작 전에 주소를 채운다"가 아니라 **"쓸 때 없으면 그때 묻는다"**로
잡았다. 앞의 방식은 경로가 늘어날 때마다 또 빠뜨린다.

## v3.01

### ffmpeg 필터에는 작은따옴표가 들어간 경로를 넣을 수 없다

`IU 'Through the Night'` 같은 제목이면 자막 필터가 깨져 영상이 안 만들어졌다.
`subtitles='<경로>'`에서 ffmpeg가 따옴표를 벗겨내 `IU Through the Night`를 찾는다.

이스케이프 네 가지(`\'`, `\\'`, `'\''`, 따옴표 없이)를 모두 실측했는데 전부 실패했다.
필터 문자열은 두 단계로 언이스케이프되고, 그 문법에 작은따옴표를 넣는 방법이 없다.

그래서 **경로를 아예 안 넘긴다.** 자막과 글꼴을 임시 폴더에 두고 ffmpeg의 작업
디렉터리를 그리로 옮긴 뒤 `subtitles=가사.ass:fontsdir=.`처럼 고정된 이름만 쓴다.
이름에 특수문자가 낄 자리가 없어진다. 결과 MP4 경로는 argv로 넘어가므로 무관하다.

### 가사를 언제 조회하느냐가 UI를 결정했다

가사가 없으면 노래방 영상이 의미가 없는데, 조회를 변환 도중에 하면 못 찾았을 때
물어볼 방법이 없다. 팝업을 띄우면 대기열이 거기서 멈춘다.

그래서 **분리 시작을 누른 직후**로 옮겼다. 사용자가 어차피 기다리는 순간이고,
확보한 가사를 항목에 물고 있다가 변환 때 그대로 쓴다(재조회 없음).

대기열 행에 가사 상태를 표시하는 안도 검토했지만 접었다. 잘 찾는 곡이 대부분인데
모든 행에 열이 하나 늘고, 가사 길이가 제각각이라 뒤 요소들이 밀려 정렬이 깨진다.
**못 찾았을 때만 팝업**이면 평소 화면은 지금과 완전히 같다.

길이 대조는 유지해야 한다(v2.05·v2.06에서 오매칭 막으려고 넣은 것). 오디오를 받기
전이라 파일에서 길이를 못 재므로, 제목 확인할 때 yt-dlp가 주는 `duration`을 같이 저장한다.

### 싱크 가사를 직접 고르게 하니 길이 검사가 뚫렸다

라이브 영상(286초)에 스튜디오 LRC(279초)를 얹어 자막이 12초 밀렸다. 자동 조회는
원래 길이를 ±4초로 대조하는데(v2.05에서 오매칭 막으려고 넣음), 팝업에서 직접 고르는
경로가 그 검사를 건너뛰었다. 기능을 늘릴 때 **기존 검증을 우회하는 경로가 생기지
않는지** 봐야 한다.

처음에는 "노래 시작 시각(vocal_onset)과 LRC 첫 줄을 비교"로 접근했는데 실측에서 버렸다.
라이브 보컬 스템은 관객 소리와 MC 멘트가 섞여 있어 onset이 안 나온다.

    0~2초    12~13%   관객
    6~9초    39~44%   노래 구간보다 크다
    19초~     8~32%   실제 노래

임계값 3종 × 지속프레임 5종을 다 돌려도 19초를 집는 조합이 없었다. 6~9초 덩어리가
항상 먼저 걸리거나, 조건을 올리면 35초·66초로 넘어간다. 파형 기반 onset은 라이브에서
쓸 수 없다고 보는 게 맞다.

길이 대조는 계산이 필요 없고 이미 검증된 규칙이다. 신호가 이미 있는데 새 신호를
만들려 한 게 잘못이었다.

### 같은 제목의 다른 곡 — 가수 없이 찾는 폴백이 뚫렸다

`Graham Bonnet Band - ＂Since You've Been Gone＂`에 전혀 다른 곡의 가사가 붙었다.

`fetch_lyrics`는 가수를 붙인 검색이 실패하면 **제목만으로 한 번 더** 찾는다(제목에서
가수를 잘못 뽑았을 수 있으므로). 그런데 이 제목은 Rainbow·Kelly Clarkson·Aretha
Franklin·Slank·Luther Vandross가 각각 **다른 곡**으로 갖고 있다. `_norm`이 기호를
지우므로 제목이 전부 같아지고, 남는 조건은 길이 ±4초뿐이라 아무거나 걸린다.

폴백을 없애면 가수명 없는 업로드에서 멀쩡한 가사까지 놓친다. 그래서 **모호할 때만**
조인다 — 제목·길이를 통과한 후보의 가수가 둘 이상이면, 그 가수가 원제목 안에 있는
것만 받아들인다. 후보가 하나뿐이면 예전 그대로다.

부수적으로 전각 따옴표 `＂`가 검색어에 남아 있었고, 아포스트로피를 공백으로 바꾸는
바람에 `You've`가 `You ve`로 쪼개지고 있었다.
